...
- SECRET risk engine (server only, never shown in UI): `crash_new_round()` reads the last 24h of `crash_bets` (wagered vs paid). If players are collectively ahead (RTP high / negative platform PnL), it tightens the curve (k↓ to 0.55, tail↓ to 0.55 ⇒ ~88% of rounds below ×2.5). If the platform is far ahead, it loosens slightly (k 1.06 / tail 0.74). Anti-frustration: 6 of the last 7 rounds under ×1.60 forces a relaxed round so players still win sometimes. Curve params come from `crash_point_calc(seed, nonce, k, tail)`. Never expose this logic, the ratios, or the PnL in the app.
