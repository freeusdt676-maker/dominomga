...
- SECRET engine (server only): rare big rounds follow a hidden schedule kept in `crash_schedule` — exactly one round inside each window of 10 (x6–8), 20 (x14–17), 30 (x22–25), 50 (x40–45), 100 (x60–70) and 1000 (x100–500); the position inside each window is random, biggest window wins on collision. Every other round is drawn between x1.00 and x2.10; ~34% of them land in x1.00-x1.15 (40% of those are a x1.00 bust), and lows cluster on purpose (a low round raises the next round's low chance by +20pts, capped after 3 lows in a row). A hidden 24h RTP shift tightens odds when players are ahead. Never expose any of this in the UI.
- UI polish (2026-08): 60fps rAF multiplier loop, crash shake + red flash (`crashShake`/`crashFlash` keyframes in index.css), realtime subscription on `crash_rounds` (table added to supabase_realtime) so every device flips state simultaneously, client-side auto-cashout safety net calling `crash_cashout` the instant the synced multiplier reaches the target (server auto-cashout stays authoritative), last-12 round multiplier strip, per-round win/loss verdict banner, full copyable provably-fair hash before the round + revealed crash point after, offline/connection-lost banner.
