...
- Crash point: HMAC-SHA256(server_seed, nonce) → e in [0,1); 1% instant ×1.00 bust, else `floor(0.99/(1-e)*100)/100` (1% house edge), clamped to 999.
- `crash_place_bet(_amount,_auto_cashout)`: 100–100 000 Ar, one bet per round, betting phase only, active account, respects `game_blocks('crash')`.
