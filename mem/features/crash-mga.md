...
- SECRET risk engine (server only, never shown in UI): `crash_new_round()` reads the last 24h of `crash_bets` (wagered vs paid). Baseline k=0.95 / tail=0.58 ⇒ ~10% instant bust, ~72% under ×2, ~81% under ×2.5, ~5.5% ≥×5. If players are collectively ahead (RTP high / negative platform PnL) it tightens (down to k 0.70 / tail 0.48); if the platform is far ahead it loosens slightly (k 1.02 / tail 0.66). Anti-frustration: 6 of last 7 rounds under ×1.60 forces a relaxed round. No fixed periodicity: low (<×2) runs may last up to a random cap of 8–15 rounds, high (≥×2) runs are capped at 2 in a row; the old 'break 3 identical buckets' rule was removed because it produced a high result every 4th round. Mixing is probabilistic, never a fixed rule: depending on how many of the last 4 rounds were ≥×2 (1/2/3+), the next round is forced below ×2 with 15%/45%/70% chance, plus ±8% random jitter on k/tail each round. Never expose this logic, the ratios, or the PnL in the app.
