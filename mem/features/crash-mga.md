...
- SECRET engine (server only): each round is an INDEPENDENT draw from HMAC uniforms (`crash_uniform`) mapped to buckets — ~11% ×1.00, ~57% 1.01–1.99, ~19% 2.00–2.49, ~8.5% 2.50–4.99, ~3.5% 5–19.99, ~1% 20+. No streak/pattern/memory logic at all, so results cannot be studied or predicted. A hidden 24h RTP shift (±0.04–0.07 on bucket bounds) tightens odds when players are ahead. Never expose this in the UI.
