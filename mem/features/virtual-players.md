...
- Moves: edge function `domino-autoplay` — virtual turns act with a 1–5s delay, fair info only (own hand + board + opponent tile counts, tsy mahalala vato hafa). Tanjaka 70%: 70% `chooseBestBotMove` (expert), 30% hetsika olombelona tsotra (weak/random) mba handresen'ny olona tena izy indraindra (~30%).
