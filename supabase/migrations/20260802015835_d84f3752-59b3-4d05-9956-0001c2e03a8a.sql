...
BEGIN
  ...
  IF winner_score < target_score THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'challenges_from_user_profile_fkey'
  ) THEN
    ALTER TABLE public.challenges
      ADD CONSTRAINT challenges_from_user_profile_fkey
      FOREIGN KEY (from_user) REFERENCES public.profiles(user_id) ON DELETE CASCADE;
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_games_p3_status ON public.games (player3_id, status) WHERE player3_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_challenges_to_status_expiry ON public.challenges (to_user, status, expires_at DESC);