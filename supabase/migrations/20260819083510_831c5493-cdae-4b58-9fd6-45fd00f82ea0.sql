...
BEGIN
  ...
  IF wag >= 5000 THEN
    IF rtp >= 1.30 THEN       k := 0.50; tail := 0.52;
    ELSIF rtp >= 1.10 THEN    k := 0.60; tail := 0.56;
    ELSIF rtp >= 0.95 THEN    k := 0.70; tail := 0.59;
    ELSIF rtp <= 0.55 THEN    k := 0.95; tail := 0.70;
    ELSIF rtp <= 0.70 THEN    k := 0.88; tail := 0.66;
    END IF;
  END IF;

  IF pnl < 0 THEN
    k := LEAST(k, 0.62); tail := LEAST(tail, 0.56);
  END IF;
  ...
  IF dry >= 6 THEN k := GREATEST(k, 0.95); tail := GREATEST(tail, 0.68); END IF;

  -- anti-streak: two consecutive rounds >= x2 force a low round (mixing)
  ...
  IF force_low AND cp >= 2.00 THEN
    cp := floor((1.00 + (cp - floor(cp)) * 0.95) * 100.0) / 100.0;
