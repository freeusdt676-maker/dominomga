-- 1) betting window 8s -> 10s (recreate crash_new_round unchanged apart from that)
...
-- 2) place both slots atomically in one call
CREATE OR REPLACE FUNCTION public.crash_place_bet_multi(
  _amount1 numeric, _auto1 numeric DEFAULT NULL,
  _amount2 numeric DEFAULT NULL, _auto2 numeric DEFAULT NULL)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  IF _amount1 IS NOT NULL THEN r1 := public.crash_place_bet(_amount1, _auto1); END IF;
  IF _amount2 IS NOT NULL THEN r2 := public.crash_place_bet(_amount2, _auto2); END IF;
  RETURN jsonb_build_object('ok', true, 'bet1', r1, 'bet2', r2);
END $function$;

GRANT EXECUTE ON FUNCTION public.crash_place_bet_multi(numeric, numeric, numeric, numeric) TO authenticated;