...
DECLARE
  ...
  b0a numeric; b0b numeric; b1 numeric; b2 numeric; b3 numeric; b4 numeric; b5 numeric; b6 numeric;
BEGIN
  ...
  b3  := 0.87 + shift;
  b4  := 0.955 + shift * 0.5;
  b5  := 0.986 + shift * 0.2;   -- 5.00 – 9.99
  b6  := 0.998;                 -- 10.00 – 29.99 (rare)
  ...
  ELSIF u < b5 THEN
    cp := floor((5.00 + v * 4.99) * 100.0) / 100.0;
  ELSIF u < b6 THEN
    cp := floor((10.00 + v * 19.99) * 100.0) / 100.0;
  ELSE
    cp := floor((30.0 / power(1.0 - v * 0.98, 0.55)) * 100.0) / 100.0;
  END IF;
  ...
  IF cp >= 100 THEN
    ...
    IF recent THEN cp := floor((10.00 + v * 19.99) * 100.0) / 100.0; END IF;
  END IF;
