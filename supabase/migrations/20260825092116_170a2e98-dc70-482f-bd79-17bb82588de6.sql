...
BEGIN
  ...
  IF NOT privileged THEN
    IF caller IS NULL OR NOT (caller=ANY(ids)) THEN RAISE EXCEPTION 'forbidden_caller'; END IF;
    IF NOT _blocked AND caller<>_winner THEN RAISE EXCEPTION 'forbidden_caller'; END IF;
  END IF;
  ...
  IF instant_win THEN settlement:=public.settle_game(_game_id,_winner); END IF;
