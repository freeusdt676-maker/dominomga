CREATE OR REPLACE FUNCTION public.is_virtual_player(_uid uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public' AS $$
  SELECT EXISTS (SELECT 1 FROM public.virtual_players v WHERE v.user_id = _uid);
$$;
...
BEGIN
  ...
  FOR i IN 1..pcount LOOP
    ...
    -- Mpilalao virtuel: tsy mandoa vola (ny système no manohana azy)
  ...
  pot := (g.stake - commission_each) * real_count;
...
BEGIN
  IF NEW.status = 'finished' AND NEW.winner_id IS NOT NULL
    ...
    VALUES ('domino', NEW.id, NEW.ticket_number, 'settle', NEW.stake, NEW.commission,
            (NEW.stake - round(NEW.stake*0.10)) * real_count, NEW.winner_id, pc,
...
AS $function$
  ...
  WHERE public.has_role(auth.uid(), 'admin')
    ...
    AND w.user_id NOT IN (SELECT user_id FROM public.virtual_players);
$function$;
